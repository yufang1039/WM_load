...
Press any key to continue."""
            ...
            for block_idx in range(start_block_idx, len(self.block_order)):
                ...
                for trial_idx, trial_dir in enumerate(trial_dirs):
                    ...
                    # After practice trial, prompt subject to press space to start real trials
                    if is_practice and trial_idx == len(trial_dirs) - 1 == 0:
                        # Only one trial in block (all practice), skip prompt
                        pass
                    elif is_practice:
                        practice_end_msg = "Practice trial complete.\n\nPress SPACE to begin the real trials."
                        self.instruction_text.text = practice_end_msg
                        self.instruction_text.draw()
                        self.win.flip()
